...
void GPU_mul (double *A, double *B, double *C, int lda, int ldb, int ldc,
    int XA, int XB, int XC, int YA, int YB, int YC, double alpha, double beta) {
  cublasDgemm(handle, CUBLAS_OP_N, CUBLAS_OP_N, XB, YA, XA, &alpha, B, ldb, A, lda, &beta, C, ldc);
}

void GPU_add (double *A, double *B, double *C, int lda, int ldb, int ldc,
    int XA, int YA, double alpha, double beta) {
...
/*
   ...
   A = XA x YA    B = XB x YB   C = XC x YC
 */
void strassen(double *A, double *B, double *C, int lda, int ldb, int ldc,
    int XA, int XB, int XC, int YA, int YB, int YC,
    int depth) {
  ...
  a12 = A+dxa; a21 = A+dya; //a22 = A+dxa+dya;
  b12 = B+dxb; b21 = B+dyb; //b22 = B+dxb+dyb;
  c12 = C+dxc; c21 = C+dyc; //c22 = C+dxc+dyc;

  /* 
     A11 = nxa x nya    a12 = pxa x nya
     a21 = nxa x pya    a22 = pxa x pya
   */
...
int main (int argc, char **argv) {
  ...
  for (int i=0; i<sizeA; i++) h_A[i]=(i%3);
  for (int i=0; i<sizeB; i++) h_B[i]=(i%3);
  for (int i=0; i<sizeC; i++) h_C[i]=0.0f;
  for (int i=0; i<sizeC; i++) v_C[i]=0.0f;
...
//  printf ("%d %d %d %.2f\n", M,N,K,strassenTime);
  ...
  if (check) {
    ...
    if (absErr>1) printf("CHECK: Absolute error: %lf\n", absErr);
  }
